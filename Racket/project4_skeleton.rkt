...
;           Project 4, CIS505/705, 
;        Kansas State University, Fall 2019
;
; This is a skeleton, with 7 places where code needs to be changed/inserted.
; Each such place is marked with "CHANGE #k" where k is a number;
;  if you get 1&2 done you can run the functional core (from last Project).
; 
; #1: this is about how to evaluate a function definition;
;      the appropriate closure should be created
;
; #2: this is about how to evaluate a function application
;       which is done slightly different from what we did in Project 3;
;      you may take inspiration from the last slide in "MutationsInterpret"
;
; #3: this is about how to evaluate sequential composition;
;       you must make sure that the side effects of the first expression
;       are remembered when evaluating the second expression
;
; #4: this is about how to process the definition of a new class;
;       the current class environment must be extended
;       to accommodate the new definition
;
; #5: this is about how to create a new object;
;       you must let it contain an environment 
;       that binds its fields to fresh locations 
;       as provided by the auxiliary function 'new-loc!'
;
; #6: this is about how to get the value of a field;
;       the object part must indeed evaluate to an object
;        that contains an environment in which you can look up the field
;         so as to return a location which you can then look up in the store.
;
; #7: this is about how to change the value of a field;
;      the object part must indeed evaluate to an object
;       that contains an environment in which you can look up the field
;        so as to return a location which you can then modify in the store 
;         to the value of the second expression.
;     (Usually, we don't care about the value returned;
   ...
   (cond
      ...
      [(LambdaExp? exp)
           (values (NumV 101)   ;;; CHANGE #1
                    sto)
           ; return a closure and an unchanged store (sto)
           ; (ClosureV  )
           ]
      [(ApplyExp? exp)
          (let*-values 
